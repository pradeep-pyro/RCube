...
MeshData box(float width, float height, float depth, unsigned int width_segments,
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        box_data.vertices.insert(box_data.vertices.end(), data[i].vertices.begin(), data[i].vertices.end());
        box_data.indices.insert(box_data.indices.end(), data[i].indices.begin(), data[i].indices.end());
        box_data.normals.insert(box_data.normals.end(), data[i].normals.begin(), data[i].normals.end());
        box_data.texcoords.insert(box_data.texcoords.end(), data[i].texcoords.begin(), data[i].texcoords.end());
    }
    return box_data;
}
