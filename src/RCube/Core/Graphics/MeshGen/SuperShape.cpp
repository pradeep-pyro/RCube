...
float superFormula(float angle, float a, float b, float m1, float m2, float n1, float n2, float n3)
{
    ...
    using std::sin;
    float tmp =
        pow(abs(cos(m1 * angle * 0.25f) / a), n2) + pow(abs(sin(m2 * angle * 0.25f) / b), n3);
    return std::pow(tmp, -1.f / n1);
}

glm::vec3 superSphericalCoordinates(float lat, float lon, float lat_a, float lat_b, float lat_m,
                                    float lat_n1, float lat_n2, float lat_n3, float lon_a,
                                    float lon_b, float lon_m, float lon_n1, float lon_n2,
                                    float lon_n3)
{
                    ...
                    float m2, float n1, float n2, float n3)
{
    ...
    for (unsigned int i = 0; i < rows - 2; ++i)
    {
        ...
        for (unsigned int j = 0; j < cols; ++j)
        {
            ...
            glm::vec3 pt =
                superSphericalCoordinates(lat, lon, a, b, m1, n1, n2, n3, a, b, m2, n1, n2, n3);
